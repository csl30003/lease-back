...
type User struct {
	...
	Wallet   float64 `gorm:"column:wallet;type:decimal(10,2);not null;default:0;comment:钱包" json:"wallet"`
}
...
func UpdateUserWallet(id int, wallet float64) {
	database.DB.Model(&User{}).Where("id = ?", id).Update("wallet", wallet)
}
